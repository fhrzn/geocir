...
class GeoCLIP(nn.Module):
    ...
    @torch.no_grad()
    def predict_batch(self, image_paths, top_k):
        """Given multiple images, predict the top k GPS coordinates per image.
        ...
        Returns:
            top_pred_gps (torch.Tensor): Top k GPS coordinates of shape (n, k, 2),
                where n is len(image_paths).
        ...
        logits_per_image = self.forward(image_batch, gps_gallery)
        probs_per_image = logits_per_image.softmax(dim=-1).cpu()

        # Get top k predictions for each image in the batch
        top_pred = torch.topk(probs_per_image, top_k, dim=1)
